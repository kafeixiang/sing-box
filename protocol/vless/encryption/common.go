...
func (c *CommonConn) Write(b []byte) (int, error) {
	...
	for n := 0; n < len(b); {
		data := b[n:]
		if len(data) > 8192 {
			data = data[:8192] // for avoiding another copy() in peer's Read()
		}
		n += len(data)
		headerAndData := outBytes[:5+len(data)+16]
		EncodeHeader(headerAndData, len(data)+16)
		max := false
		if bytes.Equal(c.AEAD.Nonce[:], MaxNonce) {
			max = true
		}
		c.AEAD.Seal(headerAndData[:5], nil, data, headerAndData[:5])
		if max {
