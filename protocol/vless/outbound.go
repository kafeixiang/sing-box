...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VLESSOutboundOptions) (adapter.Outbound, error) {
	...
	if options.Encryption != "" && options.Encryption != "none" {
		...
		logger.Debug("encryption initialized with ", len(encryptionConfig.keys), " keys xorMode=", encryptionConfig.xorMode, " seconds=", encryptionConfig.seconds, " padding=", encryptionConfig.padding)
	}
...
func parseClientEncryption(raw string) (clientEncryptionConfig, error) {
	...
	if secondsToken == "" {
		return cfg, E.New("invalid encryption seconds segment")
	}
	trimmed := strings.TrimSuffix(secondsToken, "s")
	if trimmed == "" {
		return cfg, E.New("invalid encryption seconds segment")
	}
	seconds, err := strconv.ParseUint(trimmed, 10, 32)
	if err != nil {
		return cfg, E.Cause(err, "parse encryption seconds")
	}
	cfg.seconds = uint32(seconds)
